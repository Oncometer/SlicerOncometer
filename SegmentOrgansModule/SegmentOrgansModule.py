...
def _ensureMooseInstalled() -> None:
    if importlib.util.find_spec("moosez") is None:
...
class SegmentOrgansModuleLogic(ScriptedLoadableModuleLogic):
    ...
    def _runMooseInference(
        ...
        with tempfile.TemporaryDirectory(prefix="SegmentOrgansModule_Moose_") as tempDirectory:
            ...
            _ensureMooseInstalled()
            moosezExecutable = shutil.which("moosez")
            if not moosezExecutable:
                slicerPythonDirectory = Path(sys.executable).parent
                candidate = slicerPythonDirectory / "moosez"
                if candidate.is_file():
                    moosezExecutable = str(candidate)
            if not moosezExecutable:
                raise RuntimeError(
                    "The moosez command is not available after automatic installation. Restart Slicer and try again."
                )
