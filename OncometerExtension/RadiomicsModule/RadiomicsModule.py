...
# Optional dependency: PyRadiomics
try:
    from radiomics import featureextractor as _pyradiomics_featureextractor
    _PYRADIOMICS_AVAILABLE = True
    _PYRADIOMICS_IMPORT_ERROR = ""
except Exception as _pyradiomics_error:  # noqa: BLE001 - broad to stay compatible with missing native deps
    _pyradiomics_featureextractor = None
    _PYRADIOMICS_AVAILABLE = False
    _PYRADIOMICS_IMPORT_ERROR = str(_pyradiomics_error)
...
class RadiomicsModuleWidget(ScriptedLoadableModuleWidget, VTKObservationMixin):
    ...
    def setup(self) -> None:
        ...
        # If PyRadiomics is not available, disable first-order features in the UI.
        # This keeps the module usable for non-PyRadiomics metrics.
        if not _PYRADIOMICS_AVAILABLE:
            logging.info(
                "PyRadiomics is not available; FirstOrder features will be disabled. ImportError=%s",
                _PYRADIOMICS_IMPORT_ERROR,
            )
            try:
                self.ui.checkBox_FirstOrder.setChecked(False)
                # Prefer Qt API (works across bindings)
                if hasattr(self.ui.checkBox_FirstOrder, "setEnabled"):
                    self.ui.checkBox_FirstOrder.setEnabled(False)
                else:
                    self.ui.checkBox_FirstOrder.enabled = False
                self.ui.checkBox_FirstOrder.toolTip = _(
                    "Premier ordre indisponible : PyRadiomics (package 'pyradiomics') n'est pas installé dans Python de Slicer."
                )
            except Exception:
                # UI attribute names can vary slightly across Slicer/Qt bindings.
                pass
...
class RadiomicsModuleLogic(ScriptedLoadableModuleLogic):
    ...
    def process(self,
        ...
        if selectedMetrics.get("FirstOrder"):
            ...
            if not _PYRADIOMICS_AVAILABLE or _pyradiomics_featureextractor is None:
                logging.warning(
                    "FirstOrder requested but PyRadiomics is not available; skipping. ImportError=%s",
                    _PYRADIOMICS_IMPORT_ERROR,
                )
            else:
                firstOrderExtractor = _pyradiomics_featureextractor.RadiomicsFeatureExtractor(binWidth=0.25)
